"""
hello_postgresql — PostgreSQL 数据库示例

与 hello_world 完全相同的爬虫逻辑，唯一区别：
通过 db_type="postgresql" 切换数据库后端为 PostgreSQL。

前置条件：
    pip install psycopg2-binary
    创建 PostgreSQL 数据库 jimmyspider（或修改 jimmyspider.yaml 中的 pg_db）
    表结构由 PostgreSQLHandler 自动创建
"""
...
class Spider(JimmySpider):
    ...
    def run(self):
        ...
        self.log_print.info("PostgreSQL 示例完成")
